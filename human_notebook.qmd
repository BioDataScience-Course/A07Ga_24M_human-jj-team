...
```{r Thea descript 1}
chart(biometry, tour_taille ~tour_hanche %shape=% genre %col=% genre) +
  geom_point()
```
...
```{r Lea descript 2}
chart(data = biometry, age ~ sommeil %fill=% genre | genre ) +
  geom_violin()
```
...
-   Nous pouvons constater que les femmes boivent plus d'eau que les hommes.

-   On peut voir qu'en moyenne, hommes et femmes confondus boivent 1.5 litres d'eau par jour.
