...
Pour effectuer nos analyses, chacun des expérimentateurs ont récoltés différentes données sur quatre sujets volontaires différents, ce qui nous a permis d'obtenir un nombre de données suffisant pour effectuer une analyse correcte. Les variables mesurées par les expérimentateurs comprenaient notamment des paramètres physique tel que la masse, la taille, les mensurations mais aussi des mesures telles que le genre, l'alimentation, le sommeil,... et ont été retranscrite dans un disctionnaire de données commun à tous les expérimentateurs.
